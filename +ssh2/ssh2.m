...
elseif (input_check == 2) %allow ssh2 to be interface to ssh_simpleCommand
    ...
    if nargout == 0
        ssh2.ssh2_simple_command(hostname, username, password, command, enableprint);
    else
        ssh2_struct = ssh2.ssh2_simple_command(hostname, username, password, command, enableprint);
    end
else
    ssh2_struct = ssh2.ssh2_main(ssh2_struct);    
end